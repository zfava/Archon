...
|---|---|---|---|---|
| R2 | **Core state lost on restart** | 33 PostgreSQL-backed stores (31 via central persistence layer + 2 via domain-specific modules) implemented via `ArchonAI.Persistence`, `ArchonAI.Telemetry`, and `ArchonAI.Knowledge` layers with config-driven factory pattern. DbUp migration framework with 26 numbered scripts (001–026). | `DependencyInjection.cs` — 31 `ReplaceWithFactory` registrations (22 domain + 9 identity), plus `PostgresTaskTelemetryStore` and `PostgresKnowledgeGraphStore` in domain-specific modules. 18 multi-instance Testcontainers integration tests. | All identified persistence gaps now closed. R2-residual resolved. |
...
| R6 | **No database migration framework** | DbUp adopted. 26 sequential SQL scripts (001–026) with transaction-per-script, journal table (`schemaversions`), health check for pending migrations, and complete rollback scripts (26 down scripts). | `MigrationRunner.cs`, `MigrationHealthCheck.cs`, `Scripts/001–026_*.sql`, `Down/*.sql`. | Implemented in source and integrated into API startup. Not yet validated through a real schema evolution cycle in production. |
...
| R17 | **No data retention policies** | Configurable retention: audit 730d, traces 90d, telemetry 90d, inspection 90d, session memory 240h. `RetentionHostedService` runs daily at 02:00 UTC. Retention now includes inspection table sweeps (policy evaluations, memory references, workflow diagnostics) with a configurable 90-day default, added via migration 026. Retention log table tracks sweeps including inspection_rows_deleted. | `RetentionHostedService.cs`, `PersistenceOptions.cs`, migration `020_create_retention_log.sql`, migration `026_alter_retention_log_add_inspection.sql`. | Implemented in source. Not yet validated through a full retention cycle in production. |
...
| R23 | **SOQL injection surface in SalesforceConnector** | `SanitizeSoqlFilter` method added: rejects semicolons, comments (`--`, `/* */`), DML keywords (INSERT/UPDATE/DELETE/UPSERT/UNDELETE/MERGE), strips trailing LIMIT clauses. All three query methods (Accounts, Contacts, Opportunities) route through sanitizer. | `SalesforceConnector.cs` — sanitizer + 3 query methods updated. 7 new tests in `SalesforceConnectorTests.cs` covering valid filter, null filter, semicolons, comments, block comments, DML keywords, LIMIT injection. | Runtime-proven via 7 dedicated injection tests. |
| R24 | **Sync-over-async in ControlPlaneService and MemoryCompressionEngine** | `GetStatusAsync` added to `IControlPlaneService` interface and `ControlPlaneService`. All async callers (`GetDashboardAsync`, `GetUnifiedDashboardAsync`) migrated to `await GetStatusAsync(ct)`. `MemoryCompressionEngine.DeduplicateInternalAsync` made properly async with `await` instead of `GetAwaiter().GetResult()`. | `IControlPlaneService.cs`, `ControlPlaneService.cs`, `ControlPlaneObservabilityService.cs`, `MemoryCompressionEngine.cs`. | Sync `GetStatus()` preserved as thin wrapper for backward compatibility. |
| R25 | **Empty projects (EventBus, Planner) in solution** | Removed `ArchonAI.EventBus` and `ArchonAI.Planner` from `ArchonAI.slnx` and `ArchonAI.Infrastructure.csproj` ProjectReferences. IEventBus lives in Core; NatsEventBus in Infrastructure. StrategicPlanningEngine fills the planning role. | `ArchonAI.slnx`, `ArchonAI.Infrastructure.csproj`. Source project count reduced from 61 to 59. | Project directories retained for historical reference. |
| R26 | **ISecretProvider lacks async method** | Default interface method `GetSecretAsync(string key, CancellationToken ct)` added to `ISecretProvider`. Delegates to sync `GetSecret` by default. Vault providers (AWS, Azure, HashiCorp) with existing private async methods take precedence via C# DIM resolution. | `ISecretProvider.cs`. No implementing class changes required. | Future-proofs interface for async-first callers without breaking 10+ existing sync consumers. |
| R27 | **Near-zero frontend test coverage** | Contract/type-shape tests added for 3 governance-critical features: action-safety (5 tests), proof-analytics (8 tests), inspection (5 tests). Follows existing trust-lineage-contracts pattern. Vitest setup file created with `@testing-library/jest-dom`. | `action-safety-contracts.test.ts`, `proof-analytics-contracts.test.ts`, `inspection-contracts.test.ts`, `test-setup.ts`, `vite.config.ts`. 31 total frontend tests across 4 files. | Data contract tests only — component rendering tests deferred to next pass. |
| R2-residual | **Agent Registry and Control Plane not database-backed** | PostgreSQL-backed stores implemented for Agent Registry, Control Plane, Agent Capability Registry, and Control Plane Alert Store. Migrations 021–026. 18 Testcontainers multi-instance integration tests. | `PostgresAgentRegistryStore.cs`, `PostgresControlPlaneStore.cs`, `PostgresAgentCapabilityRegistryStore.cs`, `PostgresControlPlaneAlertStore.cs`. `DependencyInjection.cs` — 31 central stores (22 domain + 9 identity) plus 2 domain-specific stores (TaskTelemetry, KnowledgeGraph) = 33 total PostgreSQL-backed stores. | `PlanningFeedbackStore` remains in-memory (low-value, regenerated from runtime telemetry). |
...
| No silent data loss | **Met** — 33 PostgreSQL-backed stores (31 via central persistence layer + 2 via domain-specific modules). Unconfigured providers return `IsSuccess: false`, not fabricated data. |
...
| Zero warnings / failures | **Met** — 0 warnings, all unit tests pass (12 test assemblies, 1,285 tests). 685 enterprise tests pass. Frontend: 0 TypeScript errors, 0 lint errors, 0 build errors, 31 vitest tests pass. |
...
ArchonAI has resolved 21 of 28 tracked risks (including R2-residual, R23–R27) through source-level implementation with automated test coverage. The 7 remaining risks are operational — requiring deployment-time configuration (API keys, vault credentials), execution of existing test infrastructure (load baselines, live IdP validation), or external sandbox accounts (connector validation). The platform now includes 33 PostgreSQL-backed stores (31 via central persistence layer + 2 via domain-specific modules, including Agent Registry, Control Plane, Agent Capability Registry, and Control Plane Alerts), OIDC federation with JIT provisioning, TOTP/WebAuthn MFA with health-gated encryption enforcement, three vault-backed secret providers (HashiCorp Vault, AWS Secrets Manager, Azure Key Vault), Polly circuit breakers, DbUp migrations (26 scripts with rollbacks), worker health endpoints, Grafana dashboards, Trivy/dependency scanning, TLS-enforced database connections, data retention automation, GDPR data subject rights, and CORS adversarial hardening (12 test vectors). Multi-instance correctness is proven by 18 Testcontainers integration tests. Archon9 pass added event-driven inspection wiring and proof analytics auto-emission via GovernanceEventSubscriber (8 event types on IEventBus), executive command inline metrics (ProofBrief, ActionSafetyBrief, WorkflowBrief), deep-link query parameters across all governed operation views, retry-from-step capability in workflow diagnostics, and enterprise memory API documentation mapping 3 route groups to the six-layer model. Archon10 pass added real-time inspection streaming via SignalR InspectionHub (broadcasting from GovernanceEventSubscriber with frontend useInspectionHub hook) and action safety auto-classification (keyword-based inference with 5 categories, GatedActionExecutor integration, 24 test cases). All 8 of 8 post-elite gaps are now CLOSED.
